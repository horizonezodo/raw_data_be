package ngvgroup.com.loan.feature.interest_process.bpm.adjust;
...
@Component
@ProcessStarterSubscription(LoanVariableConstants.PREFIX_INTEREST_EDIT)
public class InterestAdjustStarter extends AbstractProcessStarter<InterestProfileDTO> {
    ...
    public InterestAdjustStarter(BpmFeignClient bpmFeignClient,
                                 FileService fileService,
                                 CommonFeignClient commonFeignClient,
                                 DocumentTemplateService templateService,
                                 InterestValidationService interestValidationService,
    ...
    protected String generateAndAttachToBusinessData(StartRequest<InterestProfileDTO> dto) {
        String processInstanceCode = interestTransactionService.getSequence().generateRegistration(LoanVariableConstants.PREFIX_INTEREST_EDIT);
    ...
    protected void saveBusinessData(String processDefinitionKey, InterestProfileDTO businessData) {
        interestTransactionService.createInterest(businessData, businessData.getId());
    }

    @Override
    protected ProcessData prepareProcessData(StartRequest<InterestProfileDTO> dto) {
        return interestBpmService.buildProcessData(dto, LoanVariableConstants.INTEREST_EDIT_TXN_CONTENT_CODE);
    }
}
