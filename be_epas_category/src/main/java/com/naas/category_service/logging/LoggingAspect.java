package com.naas.category_service.logging;
...
public class LoggingAspect {
     ...
     */
    @Pointcut(
            "within(com.naas.category_service.controller..*)" + " || within(com.naas.category_service.service..*)"
    )
            ...
            if (log.isInfoEnabled()) {
                if (result instanceof ResponseEntity<?> responseEntity) {
                    if (responseEntity.getBody() instanceof ByteArrayResource) {
                        logData.put("body", "Binary data (not logged)");
                    } else {
                        logData.put("result", mapper.valueToTree(result));
                    }
                }
                log.info("Exit: {}", logData);
            }
