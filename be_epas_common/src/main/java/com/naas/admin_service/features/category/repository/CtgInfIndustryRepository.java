...
public interface CtgInfIndustryRepository extends JpaRepository<CtgInfIndustry, Long> {
    @Query("""
    select new com.naas.admin_service.features.category.dto.CtgInfIndustryResponse(
    ...
    where (:filter is null or 
        ...
        (lower(:filter) like LOWER('hiệu lực') and cii.isActive = 1) or
        (lower(:filter) like LOWER('hết hiệu lực') and cii.isActive = 0)
    )
...
""")
    ...
    @Query("""
    SELECT new com.naas.admin_service.features.category.dto.CtgInfIndustryDto(
        c.industryCode,
        c.industryName
    )FROM CtgInfIndustry c
""")
    List<CtgInfIndustryDto> getAllData();
    ...
    void updateCtgInfIndustry(@Param("isActive") Integer isActive,
