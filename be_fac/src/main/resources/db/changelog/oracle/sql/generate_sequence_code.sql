CREATE OR REPLACE PROCEDURE GENERATE_NEXT_SEQUENCE (
    p_org_code       IN  VARCHAR2 DEFAULT '%',
...
BEGIN
DELETE FROM COM_INF_SEQUENCE
WHERE PERIOD_VALUE IS NOT NULL
  AND LENGTH(PERIOD_VALUE) = 6 -- Chỉ xóa các record dạng ngày YYMMDD
  AND TO_DATE(PERIOD_VALUE, 'RRMMDD') < TRUNC(SYSDATE) - 2
  AND UPPER(ORG_CODE) = UPPER(p_org_code)
  AND UPPER(TABLE_NAME) = UPPER(p_table_name)
  AND UPPER(FIELD_NAME) = UPPER(p_column_name)
...
BEGIN
SELECT CURRENT_SEQ INTO v_stt
FROM (
         SELECT CURRENT_SEQ
         FROM COM_INF_SEQUENCE
         WHERE UPPER(ORG_CODE) = UPPER(p_org_code)
           AND UPPER(TABLE_NAME) = UPPER(p_table_name)
           AND UPPER(FIELD_NAME) = UPPER(p_column_name)
           AND (p_prefix IS NULL OR UPPER(PREFIX) = UPPER(p_prefix))
           AND PERIOD_VALUE = v_date_key -- So sánh với key đã xử lý
...
END IF;
    ...
    IF v_found THEN
UPDATE COM_INF_SEQUENCE
SET CURRENT_SEQ = v_stt + p_num_to_gen - 1
WHERE UPPER(ORG_CODE) = UPPER(p_org_code)
  AND UPPER(TABLE_NAME) = UPPER(p_table_name)
  AND UPPER(FIELD_NAME) = UPPER(p_column_name)
  AND (p_prefix IS NULL OR UPPER(PREFIX) = UPPER(p_prefix))
  AND PERIOD_VALUE = v_date_key; -- Update theo key 'NO_DATE' hoặc '250312'
ELSE
        INSERT INTO COM_INF_SEQUENCE (
            ORG_CODE,
            TABLE_NAME,
            FIELD_NAME,
            PREFIX,
